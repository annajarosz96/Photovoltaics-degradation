...
# Perform the square regression analysis
model <- lm(DC_power ~ module_temperature + I(module_temperature^2), data = quad_reg)
...
# Predict the DC power using the square regression model
predicted_DC_power <- predict(model, newdata = data.frame(module_temperature = new_module_temp))
...
ggplot(df, aes(x = module_temperature, y = DC_power)) +
...
# Visualize residuals and display statistics
residuals <- residuals(model)
residual_stats <- quantile(residuals, probs = c(0, 0.25, 0.5, 0.75, 0.95, 1))
